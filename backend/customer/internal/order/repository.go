...
const (
	...
	QUERY_GET_ORDER_WAITING_PAYMENT_BY_CUSTOMER_ID = "SELECT * FROM `orders` WHERE orders.status = 'WaitingPayment' AND orders.customer_id = ?;"
...
func (r *repository) CreatePayment(ctx context.Context, order_id int, imagePath string) (err error) {
	dbq.Tx(ctx, r.db, func(tx interface{}, Q dbq.QFn, E dbq.EFn, txCommit dbq.TxCommit) {
		_, err := E(ctx, QUERY_CREATE_PAYMENT, nil,
			order_id,
			imagePath,
		)
		if err != nil {
...
func (r *repository) GetOrderWaitingPaymentByCustomerID(ctx context.Context, customerID int) (order *Order, _ error) {
	args := []interface{}{customerID}

	result := dbq.MustQ(ctx, r.db, QUERY_GET_ORDER_WAITING_PAYMENT_BY_CUSTOMER_ID, OptsOrderSR, args)
	if result == nil {
		return nil, sql.ErrNoRows
	}
	order = result.(*Order)
